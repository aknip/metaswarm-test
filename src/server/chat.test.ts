...
describe('executeTool (SUC-9)', () => {
  ...
  it('CH-4a: list_todos returns all todos', () => {
    repo.create('Task 1')
  ...
  it('CH-4f: update_todo preserves existing fields when not provided', () => {
    const todo = repo.create('Keep')
    const result = JSON.parse(executeTool(repo, 'update_todo', { id: todo.id }))
    expect(result.title).toBe('Keep')
    expect(result.completed).toBe(0)
  })

  it('CH-4g: update_todo returns error for missing todo', () => {
    const result = JSON.parse(
  ...
  it('CH-4h: delete_todo deletes and returns success', () => {
...
describe('Chat Router (SUC-7)', () => {
  ...
  it('CH-1: returns 400 if sessionId or message missing', async () => {
  ...
  it('CH-2: returns text response for simple message', async () => {
    const mockClient = makeMockClient([
      {
  ...
  it('CH-3: handles tool_use loop', async () => {
  ...
  it('CH-3b: handles multiple tool calls in one response', async () => {
    const mockClient = makeMockClient([
      {
        stop_reason: 'tool_use',
        content: [
  ...
  it('CH-5: preserves session history across calls', async () => {
    const mockClient = makeMockClient([
      { stop_reason: 'end_turn', content: [{ type: 'text', text: 'First reply' }] },
      { stop_reason: 'end_turn', content: [{ type: 'text', text: 'Second reply' }] },
    ])
    const router = createChatRouter({ client: mockClient as any, repo })
